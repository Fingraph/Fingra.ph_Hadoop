...
public class ComponentCountryStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentCountryStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getComponentcountry());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getComponentcountry());
    ...
    static class ComponentCountryMapper
        extends Mapper<LongWritable, Text, ComponentCountryKey, ComponentCountryEntity> {
        ...
        private ComponentCountryKey out_key = new ComponentCountryKey();
        private ComponentCountryEntity out_val = new ComponentCountryEntity();
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getComponentkey(),
                            compoparser.getCountry(), compoparser.getToken(), 
    ...
    static class ComponentCountryReducer
        extends Reducer<ComponentCountryKey, ComponentCountryEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ComponentCountryKey key, Iterable<ComponentCountryEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ComponentCountryEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/componentkey/country
                // - and order by appkey/componentkey/country/token/session
    ...
    private static class ComponentCountryPartitioner
        extends Partitioner<ComponentCountryKey, ComponentCountryEntity> {
        @Override
        public int getPartition(ComponentCountryKey key, ComponentCountryEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.componentkey+key.country).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ComponentCountrySortComparator
        extends WritableComparator {
        protected ComponentCountrySortComparator() {
            super(ComponentCountryKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ComponentCountryKey k1 = (ComponentCountryKey) w1;
            ComponentCountryKey k2 = (ComponentCountryKey) w2;
    ...
    private static class ComponentCountryGroupComparator
        extends WritableComparator {
        protected ComponentCountryGroupComparator() {
            super(ComponentCountryKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ComponentCountryStatistic(), args);
            
            WorkLogger.log(ComponentCountryStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ComponentCountryStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentCountryStatistic.class.getSimpleName()
