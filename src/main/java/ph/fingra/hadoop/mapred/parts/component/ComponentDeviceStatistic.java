...
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentDeviceEntity;
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentDeviceKey;

public class ComponentDeviceStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentDeviceStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getComponentdevice());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getComponentdevice());
    ...
    static class ComponentDeviceMapper
        extends Mapper<LongWritable, Text, ComponentDeviceKey, ComponentDeviceEntity> {
        ...
        private ComponentDeviceKey out_key = new ComponentDeviceKey();
        private ComponentDeviceEntity out_val = new ComponentDeviceEntity();
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getComponentkey(),
                            compoparser.getDevice(), compoparser.getToken(), 
    ...
    static class ComponentDeviceReducer
        extends Reducer<ComponentDeviceKey, ComponentDeviceEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ComponentDeviceKey key, Iterable<ComponentDeviceEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ComponentDeviceEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/component/device
                // - and order by appkey/component/device/token/session
    ...
    private static class ComponentDevicePartitioner
        extends Partitioner<ComponentDeviceKey, ComponentDeviceEntity> {
        @Override
        public int getPartition(ComponentDeviceKey key, ComponentDeviceEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.componentkey+key.device).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ComponentDeviceSortComparator
        extends WritableComparator {
        protected ComponentDeviceSortComparator() {
            super(ComponentDeviceKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ComponentDeviceKey k1 = (ComponentDeviceKey) w1;
            ComponentDeviceKey k2 = (ComponentDeviceKey) w2;
    ...
    private static class ComponentDeviceGroupComparator
        extends WritableComparator {
        protected ComponentDeviceGroupComparator() {
            super(ComponentDeviceKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ComponentDeviceStatistic(), args);
            
            WorkLogger.log(ComponentDeviceStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ComponentDeviceStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentDeviceStatistic.class.getSimpleName()
