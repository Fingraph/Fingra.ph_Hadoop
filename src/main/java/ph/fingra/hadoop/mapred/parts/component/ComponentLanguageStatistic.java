...
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentLanguageEntity;
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentLanguageKey;

public class ComponentLanguageStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentLanguageStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getComponentlanguage());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getComponentlanguage());
    ...
    static class ComponentLanguageMapper
        extends Mapper<LongWritable, Text, ComponentLanguageKey, ComponentLanguageEntity> {
        ...
        private ComponentLanguageKey out_key = new ComponentLanguageKey();
        private ComponentLanguageEntity out_val = new ComponentLanguageEntity();
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getComponentkey(),
                            compoparser.getLanguage(), compoparser.getToken(), 
    ...
    static class ComponentLanguageReducer
        extends Reducer<ComponentLanguageKey, ComponentLanguageEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ComponentLanguageKey key, Iterable<ComponentLanguageEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ComponentLanguageEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/conponent/language
                // - and order by appkey/component/language/token/session
    ...
    private static class ComponentLanguagePartitioner
        extends Partitioner<ComponentLanguageKey, ComponentLanguageEntity> {
        @Override
        public int getPartition(ComponentLanguageKey key, ComponentLanguageEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.componentkey+key.language).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ComponentLanguageSortComparator
        extends WritableComparator {
        protected ComponentLanguageSortComparator() {
            super(ComponentLanguageKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ComponentLanguageKey k1 = (ComponentLanguageKey) w1;
            ComponentLanguageKey k2 = (ComponentLanguageKey) w2;
    ...
    private static class ComponentLanguageGroupComparator
        extends WritableComparator {
        protected ComponentLanguageGroupComparator() {
            super(ComponentLanguageKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ...
            // grouped by appkey/component/language
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ComponentLanguageStatistic(), args);
            
            WorkLogger.log(ComponentLanguageStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ComponentLanguageStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentLanguageStatistic.class.getSimpleName()
