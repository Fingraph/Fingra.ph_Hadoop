...
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentOsversionEntity;
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentOsversionKey;

public class ComponentOsversionStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentOsversionStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getComponentosversion());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getComponentosversion());
    ...
    static class ComponentOsversionMapper
        extends Mapper<LongWritable, Text, ComponentOsversionKey, ComponentOsversionEntity> {
        ...
        private ComponentOsversionKey out_key = new ComponentOsversionKey();
        private ComponentOsversionEntity out_val = new ComponentOsversionEntity();
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getComponentkey(),
                            compoparser.getOsversion(), compoparser.getToken(),
    ...
    static class ComponentOsversionReducer
        extends Reducer<ComponentOsversionKey, ComponentOsversionEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ComponentOsversionKey key, Iterable<ComponentOsversionEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ComponentOsversionEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/component/osversion
                // - and order by appkey/component/osversion/token/session
    ...
    private static class ComponentOsversionPartitioner
        extends Partitioner<ComponentOsversionKey, ComponentOsversionEntity> {
        @Override
        public int getPartition(ComponentOsversionKey key, ComponentOsversionEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.componentkey+key.osversion).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ComponentOsversionSortComparator
        extends WritableComparator {
        protected ComponentOsversionSortComparator() {
            super(ComponentOsversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ComponentOsversionKey k1 = (ComponentOsversionKey) w1;
            ComponentOsversionKey k2 = (ComponentOsversionKey) w2;
            
            // ordered by OsversionKey compareTo
    ...
    private static class ComponentOsversionGroupComparator
        extends WritableComparator {
        protected ComponentOsversionGroupComparator() {
            super(ComponentOsversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ...
            ret = k1.osversion.compareTo(k2.osversion);
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ComponentOsversionStatistic(), args);
            
            WorkLogger.log(ComponentOsversionStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ComponentOsversionStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentOsversionStatistic.class.getSimpleName()
