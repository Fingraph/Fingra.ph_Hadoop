...
import ph.fingra.hadoop.mapred.parts.distribution.domain.ResolutionEntity;
import ph.fingra.hadoop.mapred.parts.distribution.domain.ResolutionKey;

public class ResolutionStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ResolutionStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getResolution());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getResolution());
    ...
    static class ResolutionMapper
        extends Mapper<LongWritable, Text, ResolutionKey, ResolutionEntity> {
        ...
        private ResolutionKey out_key = new ResolutionKey();
        private ResolutionEntity out_val = new ResolutionEntity();
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (commonparser.hasError() == false) {
                    ...
                    out_key.set(commonparser.getAppkey(), commonparser.getResolution(),
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getResolution(),
    ...
    static class ResolutionReducer
        extends Reducer<ResolutionKey, ResolutionEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ResolutionKey key, Iterable<ResolutionEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ResolutionEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/resolution
                // - and order by appkey/resolution/token/session
    ...
    private static class ResolutionPartitioner
        extends Partitioner<ResolutionKey, ResolutionEntity> {
        @Override
        public int getPartition(ResolutionKey key, ResolutionEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.resolution).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ResolutionSortComparator
        extends WritableComparator {
        protected ResolutionSortComparator() {
            super(ResolutionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ResolutionKey k1 = (ResolutionKey) w1;
            ResolutionKey k2 = (ResolutionKey) w2;
    ...
    private static class ResolutionGroupComparator
        extends WritableComparator {
        protected ResolutionGroupComparator() {
            super(ResolutionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ResolutionStatistic(), args);
            
            WorkLogger.log(ResolutionStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ResolutionStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ResolutionStatistic.class.getSimpleName()
