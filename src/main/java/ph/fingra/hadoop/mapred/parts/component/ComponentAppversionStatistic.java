...
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentAppversionEntity;
import ph.fingra.hadoop.mapred.parts.component.domain.ComponentAppversionKey;

public class ComponentAppversionStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentAppversionStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getComponentappversion());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getComponentappversion());
    ...
    static class ComponentAppversionMapper
        extends Mapper<LongWritable, Text, ComponentAppversionKey, ComponentAppversionEntity> {
        ...
        private ComponentAppversionKey out_key = new ComponentAppversionKey();
        private ComponentAppversionEntity out_val = new ComponentAppversionEntity();
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getComponentkey(), 
                            compoparser.getAppversion(), compoparser.getToken(), 
    ...
    static class ComponentAppversionReducer
        extends Reducer<ComponentAppversionKey, ComponentAppversionEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(ComponentAppversionKey key, Iterable<ComponentAppversionEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (ComponentAppversionEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/componentkey/appversion
                // - and order by appkey/compnentkey/appversion/token/session
    ...
    private static class ComponentAppversionPartitioner
        extends Partitioner<ComponentAppversionKey, ComponentAppversionEntity> {
        @Override
        public int getPartition(ComponentAppversionKey key, ComponentAppversionEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.componentkey+key.appversion).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class ComponentAppversionSortComparator
        extends WritableComparator {
        protected ComponentAppversionSortComparator() {
            super(ComponentAppversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            ComponentAppversionKey k1 = (ComponentAppversionKey) w1;
            ComponentAppversionKey k2 = (ComponentAppversionKey) w2;
    ...
    private static class ComponentAppversionGroupComparator
        extends WritableComparator {
        protected ComponentAppversionGroupComparator() {
            super(ComponentAppversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new ComponentAppversionStatistic(), args);
            
            WorkLogger.log(ComponentAppversionStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(ComponentAppversionStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentAppversionStatistic.class.getSimpleName()
