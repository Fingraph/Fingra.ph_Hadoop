...
import ph.fingra.hadoop.mapred.parts.distribution.domain.OsversionEntity;
import ph.fingra.hadoop.mapred.parts.distribution.domain.OsversionKey;

public class OsversionStatistic extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(OsversionStatistic.class.getSimpleName()
                + " : [run mode] " + opt_mode
        ...
        outputPath = new Path(hfsPath.getOsversion());
        ...
        copier.dirToFile(outputPath.toString(), lfsPath.getOsversion());
    ...
    static class OsversionMapper
        extends Mapper<LongWritable, Text, OsversionKey, OsversionEntity> {
        ...
        private OsversionKey out_key = new OsversionKey();
        private OsversionEntity out_val = new OsversionEntity();
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            if (logtype.equals(LogParserType.CommonLog)) {
                ...
                if (commonparser.hasError() == false) {
                    ...
                    out_key.set(commonparser.getAppkey(), commonparser.getOsversion(),
                ...
                if (compoparser.hasError() == false) {
                    ...
                    out_key.set(compoparser.getAppkey(), compoparser.getOsversion(),
    ...
    static class OsversionReducer
        extends Reducer<OsversionKey, OsversionEntity, Text, Text> {
        
        private Text out_key = new Text();
        private Text out_val = new Text();
        
        @Override
        protected void reduce(OsversionKey key, Iterable<OsversionEntity> values,
            ...
            String prev_token = "";
            String prev_session = "";
            for (OsversionEntity cur_val : values) {
                
                // values :
                // - grouped by appkey/osversion
                // - and order by appkey/osversion/token/session
    ...
    private static class OsversionPartitioner
        extends Partitioner<OsversionKey, OsversionEntity> {
        @Override
        public int getPartition(OsversionKey key, OsversionEntity value,
                int numPartitions) {
            return Math.abs((key.appkey+key.osversion).hashCode() * 127) % numPartitions;
        }
    }
    
    private static class OsversionSortComparator
        extends WritableComparator {
        protected OsversionSortComparator() {
            super(OsversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
        public int compare(WritableComparable w1, WritableComparable w2) {
            OsversionKey k1 = (OsversionKey) w1;
            OsversionKey k2 = (OsversionKey) w2;
    ...
    private static class OsversionGroupComparator
        extends WritableComparator {
        protected OsversionGroupComparator() {
            super(OsversionKey.class, true);
        }
        @SuppressWarnings("rawtypes")
        @Override
    ...
    public static void main(String[] args) {
        ...
        try {
            exitCode = ToolRunner.run(new OsversionStatistic(), args);
            
            WorkLogger.log(OsversionStatistic.class.getSimpleName()
                    + " : End mapreduce job");
        }
        catch (Exception e) {
            ErrorLogger.log(OsversionStatistic.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(OsversionStatistic.class.getSimpleName()
