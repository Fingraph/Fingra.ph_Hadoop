...
import ph.fingra.hadoop.dbms.parts.component.service.ComponentUserService;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentUserServiceImpl;
...
public class ComponentUserController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentUserController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // component user
        int ret = exeComponentUser(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeComponentUser(FingraphConfig config, TargetDate target)
            throws Exception {
        
        ComponentUserService serviceIF = ComponentUserServiceImpl.getInstance();
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectComponentUserDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectComponentUserWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectComponentUserMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteComponentUserDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteComponentUserWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteComponentUserMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            List<CompoUserAll> indst_list = new ArrayList<CompoUserAll>();
            ...
            for (Componentusersession src : src_list) {
                ...
                CompoUserAll new_row = new CompoUserAll();
            ...
            try {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    ins_ret = serviceIF.insertBatchComponentUserDay(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    ins_ret = serviceIF.insertBatchComponentUserWeek(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    ins_ret = serviceIF.insertBatchComponentUserMonth(indst_list);
                }
            }
            catch (Exception e) {
                throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ComponentUserController controller = new ComponentUserController();
            ...
            ErrorLogger.log(ComponentUserController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentUserController.class.getSimpleName()
