...
import ph.fingra.hadoop.dbms.parse.component.ComponentdeviceReader;
import ph.fingra.hadoop.dbms.parse.component.domain.Componentdevice;
import ph.fingra.hadoop.dbms.parts.component.domain.CompoDeviceAll;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentDeviceService;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentDeviceServiceImpl;
...
public class ComponentDeviceController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentDeviceController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // component device
        int ret = exeComponentDevice(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeComponentDevice(FingraphConfig config, TargetDate target)
            throws Exception {
        
        ComponentDeviceService serviceIF = ComponentDeviceServiceImpl.getInstance();
        ComponentdeviceReader reader = null;
        List<String> appkey_list = null;
        
        // get component/componentdevice result
        try {
            reader = new ComponentdeviceReader(config, target);
        ...
        if (appkey_list == null || appkey_list.size() <= 0) {
            WorkLogger.log("component/componentdevice: empty data");
            return 1;
        }
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectComponentDeviceDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectComponentDeviceWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectComponentDeviceMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteComponentDeviceDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteComponentDeviceWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteComponentDeviceMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            for (String componentkey : componentkey_list) {
                ...
                List<Componentdevice> src_list = null;
                List<CompoDeviceAll> indst_list = new ArrayList<CompoDeviceAll>();
                
                // get component/componentdevice result
                try {
                    src_list = reader.getComponentdeviceResults(appkey, componentkey);
                }
                catch (IOException ioe) {
                    throw new Exception(ioe.getMessage());
                }
                ...
                for (Componentdevice src : src_list) {
                    
                    CompoDeviceAll new_row = new CompoDeviceAll();
                ...
                try {
                    if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                        ins_ret = serviceIF.insertBatchComponentDeviceDay(indst_list);
                    }
                    else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                        ins_ret = serviceIF.insertBatchComponentDeviceWeek(indst_list);
                    }
                    else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                        ins_ret = serviceIF.insertBatchComponentDeviceMonth(indst_list);
                    }
                }
                catch (Exception e) {
                    throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ComponentDeviceController controller = new ComponentDeviceController();
            ...
            ErrorLogger.log(ComponentDeviceController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentDeviceController.class.getSimpleName()
