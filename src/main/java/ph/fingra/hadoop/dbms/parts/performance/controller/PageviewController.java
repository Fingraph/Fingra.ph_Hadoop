    ...
    public int exePageview(FingraphConfig config, TargetDate target)
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                cnt = serviceIF.selectPageviewHourCountByKey(target.getYear(),
            ...
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                    serviceIF.deletePageviewHourByDate(target.getYear(),
                            target.getMonth(), target.getDay(), target.getHour());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
        ...
        try {
            if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                ins_ret = serviceIF.insertBatchPageviewHour(indst_list);
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
