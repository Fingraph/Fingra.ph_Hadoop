...
import ph.fingra.hadoop.dbms.parts.performance.service.SessionService;
import ph.fingra.hadoop.dbms.parts.performance.service.SessionServiceImpl;
...
public class SessionController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(SessionController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // session count
        int ret = exeSession(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeSession(FingraphConfig config, TargetDate target)
            throws Exception {
        
        SessionService serviceIF = SessionServiceImpl.getInstance();
        List<Usersession> src_list = null;
        List<SessionAll> indst_list = new ArrayList<SessionAll>();
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                cnt = serviceIF.selectSessionHourCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), target.getHour(), "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectSessionDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectSessionWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectSessionMonthCountByKey(target.getYear(),
                        target.getMonth(), "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                    serviceIF.deleteSessionHourByDate(target.getYear(),
                            target.getMonth(), target.getDay(), target.getHour());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteSessionDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteSessionWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteSessionMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (Usersession src : src_list) {
            ...
            SessionAll new_row = new SessionAll();
        ...
        try {
            if (target.getRunmode().equals(ConstantVars.RUNMODE_HOUR)) {
                ins_ret = serviceIF.insertBatchSessionHour(indst_list);
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                ins_ret = serviceIF.insertBatchSessionDay(indst_list);
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                ins_ret = serviceIF.insertBatchSessionWeek(indst_list);
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                ins_ret = serviceIF.insertBatchSessionMonth(indst_list);
            }
        }
        catch (Exception e) {
            throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ErrorLogger.log(SessionController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(SessionController.class.getSimpleName()
