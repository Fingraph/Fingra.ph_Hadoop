...
import ph.fingra.hadoop.dbms.parse.component.ComponentnewuserReader;
...
import ph.fingra.hadoop.dbms.parts.component.service.ComponentNewuserService;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentNewuserServiceImpl;

public class ComponentNewuserController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentNewuserController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // component newuser
        int ret = exeComponentNewuser(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeComponentNewuser(FingraphConfig config, TargetDate target)
            throws Exception {
        
        ComponentNewuserService serviceIF = ComponentNewuserServiceImpl.getInstance();
        ComponentnewuserReader reader = null;
        List<String> appkey_list = null;
        
        // get component/componentnewuser result
        try {
            reader = new ComponentnewuserReader(config, target);
        ...
        if (appkey_list == null || appkey_list.size() <= 0) {
            WorkLogger.log("component/componentnewuser: empty data");
            return 1;
        }
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectComponentNewuserDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectComponentNewuserWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectComponentNewuserMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteComponentNewuserDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteComponentNewuserWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteComponentNewuserMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            List<Componentnewuser> src_list = null;
            List<CompoNewuserAll> indst_list = new ArrayList<CompoNewuserAll>();
            
            // get component/componentnewuser result
            try {
                src_list = reader.getComponentnewuserResults(appkey);
            }
            catch (IOException ioe) {
                throw new Exception(ioe.getMessage());
            }
            ...
            for (Componentnewuser src : src_list) {
                
                CompoNewuserAll new_row = new CompoNewuserAll();
            ...
            try {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    ins_ret = serviceIF.insertBatchComponentNewuserDay(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    ins_ret = serviceIF.insertBatchComponentNewuserWeek(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    ins_ret = serviceIF.insertBatchComponentNewuserMonth(indst_list);
                }
            }
            catch (Exception e) {
                throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ComponentNewuserController controller = new ComponentNewuserController();
            ...
            ErrorLogger.log(ComponentNewuserController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentNewuserController.class.getSimpleName()
