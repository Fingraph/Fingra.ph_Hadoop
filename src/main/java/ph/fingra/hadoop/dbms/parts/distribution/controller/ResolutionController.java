...
import ph.fingra.hadoop.dbms.parse.distribution.ResolutionReader;
...
import ph.fingra.hadoop.dbms.parts.distribution.service.ResolutionService;
import ph.fingra.hadoop.dbms.parts.distribution.service.ResolutionServiceImpl;
...
public class ResolutionController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ResolutionController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // resolution user & session
        int ret = exeResolution(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeResolution(FingraphConfig config, TargetDate target)
            throws Exception {
        
        ResolutionService serviceIF = ResolutionServiceImpl.getInstance();
        List<String> appkey_list = null;
        
        // get distribute/resolution result
        try {
        ...
        if (appkey_list == null || appkey_list.size() <= 0) {
            WorkLogger.log("distribute/resolution: empty data");
            return 1;
        }
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectResolutionDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectResolutionWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectResolutionMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteResolutionDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteResolutionWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteResolutionMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            List<Resolution> src_list = null;
            List<ResolutionAll> indst_list = new ArrayList<ResolutionAll>();
            
            // get distribute/resolution result
            try {
                ResolutionReader reader = new ResolutionReader(config, target);
                src_list = reader.getResolutionResults(appkey);
            }
            catch (IOException ioe) {
                throw new Exception(ioe.getMessage());
            }
            ...
            for (Resolution src : src_list) {
                
                ResolutionAll new_row = new ResolutionAll();
            ...
            try {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    ins_ret = serviceIF.insertBatchResolutionDay(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    ins_ret = serviceIF.insertBatchResolutionWeek(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    ins_ret = serviceIF.insertBatchResolutionMonth(indst_list);
                }
            }
            catch (Exception e) {
                throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ResolutionController controller = new ResolutionController();
            ...
            ErrorLogger.log(ResolutionController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ResolutionController.class.getSimpleName()
