...
import ph.fingra.hadoop.dbms.parse.distribution.AppversionReader;
import ph.fingra.hadoop.dbms.parse.distribution.domain.Appversion;
import ph.fingra.hadoop.dbms.parts.distribution.domain.AppversionAll;
import ph.fingra.hadoop.dbms.parts.distribution.service.AppversionService;
import ph.fingra.hadoop.dbms.parts.distribution.service.AppversionServiceImpl;
...
public class AppversionController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(AppversionController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // appversion user & session
        int ret = exeAppversion(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeAppversion(FingraphConfig config, TargetDate target)
            throws Exception {
        
        AppversionService serviceIF = AppversionServiceImpl.getInstance();
        List<String> appkey_list = null;
        
        // get distribute/appversion result
        try {
            AppversionReader reader = new AppversionReader(config, target);
        ...
        if (appkey_list == null || appkey_list.size() <= 0) {
            WorkLogger.log("distribute/appversion: empty data");
            return 1;
        }
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectAppversionDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectAppversionWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectAppversionMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteAppversionDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteAppversionWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteAppversionMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            List<Appversion> src_list = null;
            List<AppversionAll> indst_list = new ArrayList<AppversionAll>();
            
            // get distribute/appversion result
            try {
                AppversionReader reader = new AppversionReader(config, target);
                src_list = reader.getAppversionResults(appkey);
            }
            catch (IOException ioe) {
                throw new Exception(ioe.getMessage());
            }
            ...
            for (Appversion src : src_list) {
                
                AppversionAll new_row = new AppversionAll();
            ...
            try {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    ins_ret = serviceIF.insertBatchAppversionDay(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    ins_ret = serviceIF.insertBatchAppversionWeek(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    ins_ret = serviceIF.insertBatchAppversionMonth(indst_list);
                }
            }
            catch (Exception e) {
                throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ErrorLogger.log(AppversionController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(AppversionController.class.getSimpleName()
