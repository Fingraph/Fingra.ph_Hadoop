...
import ph.fingra.hadoop.dbms.parse.component.ComponentusersessionReader;
import ph.fingra.hadoop.dbms.parse.component.domain.Componentusersession;
import ph.fingra.hadoop.dbms.parts.component.domain.CompoSessionAll;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentSessionService;
import ph.fingra.hadoop.dbms.parts.component.service.ComponentSessionServiceImpl;

public class ComponentSessionController {
    ...
    public int run(String[] args) throws Exception {
        ...
        WorkLogger.log(ComponentSessionController.class.getSimpleName()
                + " : [run mode] " + opt_mode
                + " , [target date] " + targetDate.getFulldate());
        
        // component session
        int ret = exeComponentSession(fingraphConfig, targetDate);
        
        return ret;
    }
    
    public int exeComponentSession(FingraphConfig config, TargetDate target)
            throws Exception {
        
        ComponentSessionService serviceIF = ComponentSessionServiceImpl.getInstance();
        ComponentusersessionReader reader = null;
        List<String> appkey_list = null;
        
        // get component/componentusersession result
        try {
            reader = new ComponentusersessionReader(config, target);
        ...
        if (appkey_list == null || appkey_list.size() <= 0) {
            WorkLogger.log("component/componentusersession: empty data");
            return 1;
        }
        ...
        try {
            ...
            if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                cnt = serviceIF.selectComponentSessionDayCountByKey(target.getYear(),
                        target.getMonth(), target.getDay(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                cnt = serviceIF.selectComponentSessionWeekCountByKey(target.getYear(),
                        target.getWeek_str(), "", "");
            }
            else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                cnt = serviceIF.selectComponentSessionMonthCountByKey(target.getYear(),
                        target.getMonth(), "", "");
            }
            
            if (cnt > 0) {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    serviceIF.deleteComponentSessionDayByDate(target.getYear(),
                            target.getMonth(), target.getDay());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    serviceIF.deleteComponentSessionWeekByDate(target.getYear(),
                            target.getWeek_str());
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    serviceIF.deleteComponentSessionMonthByDate(target.getYear(),
                            target.getMonth());
                }
            }
        }
        ...
        for (String appkey : appkey_list) {
            ...
            List<Componentusersession> src_list = null;
            List<CompoSessionAll> indst_list = new ArrayList<CompoSessionAll>();
            
            // get component/componentusersession result
            try {
                src_list = reader.getComponentusersessionResults(appkey);
            }
            catch (IOException ioe) {
                throw new Exception(ioe.getMessage());
            }
            ...
            for (Componentusersession src : src_list) {
                
                CompoSessionAll new_row = new CompoSessionAll();
            ...
            try {
                if (target.getRunmode().equals(ConstantVars.RUNMODE_DAY)) {
                    ins_ret = serviceIF.insertBatchComponentSessionDay(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_WEEK)) {
                    ins_ret = serviceIF.insertBatchComponentSessionWeek(indst_list);
                }
                else if (target.getRunmode().equals(ConstantVars.RUNMODE_MONTH)) {
                    ins_ret = serviceIF.insertBatchComponentSessionMonth(indst_list);
                }
            }
            catch (Exception e) {
                throw e;
    ...
    public static void main(String[] args) {
        ...
        try {
            ComponentSessionController controller = new ComponentSessionController();
            ...
            ErrorLogger.log(ComponentSessionController.class.getSimpleName()
                    + " : Error : " + e.getMessage());
            WorkLogger.log(ComponentSessionController.class.getSimpleName()
